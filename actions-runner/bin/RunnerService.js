...
var exitServiceAfterNFailures = Number(process.env.GITHUB_ACTIONS_SERVICE_EXIT_AFTER_N_FAILURES);
      ...
      listener.on('close', code => {
        ...
        if (code === 0) {
          ...
          if (retriableFailureRetryCount >= 10) {
            console.error('Stopping the runner after 10 consecutive re-tryable failures');
            stopping = true;
          }
          ...
          console.log('Runner listener exit with Session Conflict error, stop the service, no retry needed.');
          stopping = true;
        } else {
          var messagePrefix = 'Runner listener exit with undefined return code';
          unknownFailureRetryCount++;
          retriableFailureRetryCount = 0;
          if (!isNaN(exitServiceAfterNFailures) && unknownFailureRetryCount >= exitServiceAfterNFailures) {
            console.error(`${messagePrefix}, exiting service after ${unknownFailureRetryCount} consecutive failures`);
            stopping = true;
          } else {
